...
public class ReservationController {
    ...
    //@AuthenticationPrincipalアノテーションを引数につけ、現在ログイン中のユーザー情報を取得
    ...
    /*予約フォームの送信先（/houses/{id}/reservations/input）を担当するinput()メソッドを定義
     * input()内2つのバリデーションを行い、不備があれば
     *addError()メソッドを使ってBindingResultオブジェクトに独自のエラー内容を追加
     *①チェックイン日はチェックアウト日よりも前の日付が選択されているか②宿泊人数が定員を超えていないか*/
                        ...
                        Model model)
    {
        ...
        // セッションにDTOを保存する
        /*不備なし→ReservationInputFormオブジェクトの値や計算結果などから
         * ReservationDTOクラスのインスタンスを生成し、セッションに保存*/
        httpSession.setAttribute("reservationDTO", reservationDTO); //httpSessionセッションにデータを保存したり、取得するためのインターフェース

        return "redirect:/reservations/confirm";
    }  
    
    /*セッションからReservationDTOオブジェクトを取得して予約内容の確認ページを表示する
     * HttpSessionインターフェースのgetAttribute()メソッドでreservationDTOを取得する*/
    @GetMapping("/reservations/confirm")
    public String confirm(@AuthenticationPrincipal UserDetailsImpl userDetailsImpl,
            RedirectAttributes redirectAttributes,
            HttpSession httpSession,
            Model model) {    
        // セッションからDTOを取得する (ReservationDTO)で型変換しているのはgetAttribute()の戻り値がObject型だから
        ReservationDTO reservationDTO = (ReservationDTO)httpSession.getAttribute("reservationDTO");

        //セッションにReservationDTOオブジェクトが存在しない場合の処理
