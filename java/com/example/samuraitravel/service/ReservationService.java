...
public class ReservationService {
    ...
    public String getPreviousDates(LocalDate checkinDate, LocalDate checkoutDate, BindingResult bindingResult) { 
        ...
        //checkinDateやcheckoutDateどちらか1つでもnull・エラー
        return "";
    } 

    // 宿泊料金を計算する
    public Integer calculateAmount(LocalDate checkinDate, LocalDate checkoutDate, Integer price) {
    	//ChronoUnit.DAYS.between()を使い、2つの日付（LocalDate型）の間の日数=宿泊日数を計算
    ...
    // 予約のレコード数を取得する
    //テスト前とテスト後のレコード数を比べ、予約の登録に成功または失敗したことを確認する
    public long countReservations() {
        return reservationRepository.count();
    }

    // idが最も大きい予約を取得する
    //レコード数を取得するメソッド JpaRepositoryインターフェースを継承した時点で使えるため、自分で定義する必要なし
    public Reservation findFirstReservationByOrderByIdDesc() {
        return reservationRepository.findFirstByOrderByIdDesc();
    }   
    
    @Transactional
    //ReservationDTOオブジェクトやUserオブジェクトの代わりにsessionMetadataというMap型のオブジェクトを受け取る
    public void createReservation(Map<String, String> sessionMetadata) {    
        Reservation reservation = new Reservation();

        //sessionMetadataは支払い情報のメタデータが含まれているオブジェクト
        //get()メソッドにメタデータ名を渡すことでその値を取得できる
