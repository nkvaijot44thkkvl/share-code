...
public class StripeService {
 ...
 // Stripeのシークレットキー
    //@Valueアノテーションつけ、@Value("${プロパティ名}")と記述で、application.propertiesファイルの設定値をそのフィールドに注入
 ...
 // 決済成功時のリダイレクト先URL 
    ...
    @PostConstruct
    private void init() {
        // Stripeのシークレットキーを設定する ここに書くと危ないので別ファイルから読み込み
    	Stripe.apiKey = stripeApiKey;
    }

    //役割はStripeに送信する支払い情報をセッションとして作成すること
    //作成したセッションのIDを戻り値として返す
    //このセッションIDをStripeに送信することで、Stripe側でセッションが保持する情報を取得し、適切な決済ページを作成
    public String createStripeSession(ReservationDTO reservationDTO, User user) {
    	//宿のオブジェクトを取得 
    	//存在するかわからないけど、DTOに格納されたハウスIDでエンティティを取得
        Optional<House> optionalHouse = houseRepository.findById(reservationDTO.getHouseId());
        //存在すればオブジェクトを格納　存在しなかったらラムダ式関数でエラーを返す
 ...
 // セッションから予約情報を取得し、ReservationServiceクラスを介してデータベースに登録する
    //引数として受け取ったEventオブジェクトからStripeObjectオブジェクトを取得し、それをSessionオブジェクトに型変換
    ...
    public void processSessionCompleted(Event event) {
        ...
        //OptionalクラスのifPresentOrElse()メソッド:値が存在する場合としない場合の処理」を簡潔に書くことができる
        ...
        /*StripeのAPIとstripe-javaライブラリのバージョンの組み合わせによっては互換性がなく
		「EventオブジェクトからStripeObjectオブジェクトを取得できず、予約情報をデータベースに登録できない」といった問題が発生しうる
		 そこで上記のコードで、StripeのAPIとstripe-javaライブラリのバージョンをコンソールに出力し、問題の原因を特定しやすくした*/
    }
}